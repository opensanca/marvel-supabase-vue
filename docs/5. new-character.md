## New Character
Agora que temos a nossa estrutura pronta, precisamos a criação do nosso formuário,
para isso crie os seguintes passos:

1. Crie um diretório dentro de `views` com o nome `character`
2. Crie o template conforme script abaixo:
...
Crie a rota referente a criação de um novo personagem, acesse o arquivo `router/index.js`:

**Obs: Renomeia a sua constante de DashBoard para Characters e altere também na rota, fica mais semântico.**

```js

const Characters = () => import('../views/dashboard/index.vue');
